...
def new_map(n_cells=150, n_rivers=10, min_water_ratio=0.25, min_river_height=0.6, seed=None):
    if seed is not None:
        np.random.seed(seed)
    g = Graph(N=n_cells, iterations=2)
    assign_terrain_types_to_graph(graph=g, min_water_ratio=min_water_ratio)

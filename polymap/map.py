...
from polymap.terrain import TerrainType, BiomeType
from polymap.voronoi import VoronoiPolygons
...
class Graph:
    ...
    def plot_full_map(
        ...
        polygons = [self._center_to_polygon(center, plot_type, 
        ...
        if rivers:
            for edge in self.edges:
                if edge.river > 0:
                    ...
                    X = (beg_x, end_x)
                    Y = (beg_y, end_y)
                    ax.plot(X, Y, linewidth=2+2*np.sqrt(edge.river), color='blue')
    ...
    def _center_to_polygon(self, center, plot_type, edgecolor='black', linewidth=2):
        """
        ...
        return Polygon(np.c_[xs, ys], facecolor=color, edgecolor=edgecolor, linewidth=linewidth)
